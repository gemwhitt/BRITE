...
; Explanation: 
;
; Syntax: pks = brite_findpeaks(data,minthresh=mthr,spread=spr)
;
; Example: 
;
...
;;;
;-
FUNCTION brite_findpeaks, data, minthresh=mthr, spread=spr
...
;stop
            ...
            bb=sort(cutout)
            ;avb=median(cutout[bb[0:2*spr]]) ; original up until 30th Jan
            ;avb=median(cutout[bb[0:8]]) ; possible replacement - takes 9 pixels
            avb=median(cutout[bb[0:n_elements(cutout)-2]])
...
;stop
            ...
            ; local background 
;	stop
            IF (aa[0] EQ  mp) AND (data[i,j]-avb GT 100.) THEN BEGIN
