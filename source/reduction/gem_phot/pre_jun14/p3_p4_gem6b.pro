pro p3_p4_gem6b
  ...
  ; modified from p3_p4_gem6 - 26 March 2014 - for testsets
  ;
  ...
  indir='~/BRITE/data/UB/testing/temp_effects/p3/'
  ;indir='~/BRITE/data/UB/bert/sav/'
  filesin=file_search(indir+'Orion-CF1-*.sav', count=nfiles)
  fname=file_basename(filesin, '_p3.sav')
  
  outdir='/Users/gemmawhittaker/BRITE/data/UB/testing/temp_effects/p4/'
  
  plotdir='/Users/gemmawhittaker/BRITE/data/UB/testing/temp_effects/p4/plots/'
  
  ;outdir='/Users/gemmawhittaker/BRITE/data/UB/bert/'
  
  ;plotdir='/Users/gemmawhittaker/BRITE/data/UB/bert/plots/'
  ...
  for bb=0, nfiles-1 do begin
    ...
    ;jd, jd1, data1, ccd_temp, roi_dim, simbad_mags, p2_trend, med_trend - testsets
    
    trend=p2_trend
    
    hdname=strmid(file_basename(filesin[bb], '_p3.sav'), 12)
...
;    ra_dec1=ra_dec1[*,keep]
    ...
    bmag=strmid(simbad_mags, strpos(simbad_mags, 'B=')+2, strpos(simbad_mags, 'V=')-strpos(simbad_mags, 'B=')-2)
    
    bminv=float(bmag)-float(vmag)
...
;    ra_dec1=ra_dec1[*,keep]
    ...
    ; save results
    txtout=outdir+'txt/'+fname[bb]+'_p4gem.txt'
    savout=outdir+'sav/'+fname[bb]+'_p4gem.sav'
...
;      simbad_mags, parlax, otype, sptype, p2_trend, med_trend
    ...
    printf, lun, 'Data below: JD, Magnitudes, DN, ExpTime, CCDtemp, x_psf, y_psf, max_dn, p2_trend', format='(a)'
    for i=0, n-1 do printf, lun, jd[i], mags[i], flux[i], exp_time, ccd_temp[i], xy_psf[0,i], xy_psf[1,i], max_dn[i], p2_trend[i], $
