...
; PURPOSE - first attempt at correcting HPs - in the absense of information
; Output: make Hot pixel and CP maps for each image
...
sat='BA'
field='ORION'
opt='1' ; choose which method to use
...
for f=0, 0 do begin ;nf-1 do begin
  ...
  for orbit=0, 50 do begin  ;ngap do begin                                  ; LOOP OVER EACH ORBIT
    ...
    for img=0, ngood-1 do begin
      ...
      if opt eq '1' then begin
        ; ORIGINAL METHOD....   ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        cr2=3
        ima=find_hps2(im,thr,cr2,iloc[keep1[img]], ww,wx,wy)
    ...
    for ii=0, xdim-1 do begin  ; check every pixel in x and y
      for jj=0, ydim-1 do begin    
          ...
          if nxx ge 0.25*ngood then begin $ ; HP!
            hps[ii,jj,iloc]=hpo[ii,jj,*]
            if ii le xdim-2 then hps[ii+1,jj,iloc]=hpo[ii+1,jj,*]
          endif
      endfor
    endfor
  ...
  fileout=outdir+fname+'_hpmap'+opt+'.sav'
  print, fileout
  
  save, filename=fileout, data1, hps
